...
def rmsle(y_true, y_pred) -> float:
    ...
    return float(np.sqrt(np.mean((np.log1p(y_true) - np.log1p(y_pred_clipped)) ** 2)))


def clf_metrics(y_true_raw, y_pred_raw, split: str, target: str) -> dict:
    y_true_str = pd.Series(y_true_raw).astype(float).round(6).astype(str)
    y_pred_str = pd.Series(y_pred_raw).astype(float).round(6).astype(str)
    ...
    return {
        ...
        "macro_f1": f1_score(y_true_str, y_pred_str, labels=labels, average="macro", zero_division=0),
...
def reg_metrics(y_true_raw, y_pred_raw, y_true_log, y_pred_log, split: str) -> dict:
    return {
        "split":    split,
        "model":    "XGBoost_baseline",
...
def train_models(
    ...
    # train metrics
    cpu_tr_pred = cpu_encoder.inverse_transform(cpu_model.predict(X_train)).astype(float)
    metrics.append(clf_metrics(train["cpu_request"].values, cpu_tr_pred, "train", "cpu_request"))

    # test metrics
    cpu_te_pred = cpu_encoder.inverse_transform(cpu_model.predict(X_test)).astype(float)
    metrics.append(clf_metrics(test["cpu_request"].values, cpu_te_pred, "test", "cpu_request"))
    ...
    y_train_memory  = memory_encoder.fit_transform(train["memory_request"].astype(str))

    memory_model = Pipeline(steps=[
        ("preprocessor", preprocessor),
        ("model", XGBClassifier(
            n_estimators=150, max_depth=4, learning_rate=0.05,
            subsample=0.90, colsample_bytree=0.90,
    ...
    # train metrics
    mem_tr_pred = memory_encoder.inverse_transform(memory_model.predict(X_train)).astype(float)
    metrics.append(clf_metrics(train["memory_request"].values, mem_tr_pred, "train", "memory_request"))

    # test metrics
    mem_te_pred = memory_encoder.inverse_transform(memory_model.predict(X_test)).astype(float)
    metrics.append(clf_metrics(test["memory_request"].values, mem_te_pred, "test", "memory_request"))

    models["memory_model"]   = memory_model
    models["memory_encoder"] = memory_encoder

    # ── Duration regressor ────────────────────────────────────────────────────
    duration_numeric_features = numeric_features + ["cpu_request", "memory_request"]
    ...
    y_train_log = np.log1p(train["duration_seconds"])
    y_test_log  = np.log1p(test["duration_seconds"])
    duration_model.fit(train[duration_features], y_train_log)

    # train metrics
    dur_tr_log  = duration_model.predict(train[duration_features])
    dur_tr_pred = np.expm1(dur_tr_log).clip(min=1)
    metrics.append(reg_metrics(train["duration_seconds"].values, dur_tr_pred, y_train_log.values, dur_tr_log, "train"))

    # test metrics
    dur_te_log  = duration_model.predict(test[duration_features])
    dur_te_pred = np.expm1(dur_te_log).clip(min=1)
    metrics.append(reg_metrics(test["duration_seconds"].values, dur_te_pred, y_test_log.values, dur_te_log, "test"))
