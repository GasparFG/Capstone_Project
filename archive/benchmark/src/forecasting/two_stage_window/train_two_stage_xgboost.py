...
def create_window_features(forecast_data):
    ...
    model_data = pd.concat([model_data] + feature_blocks, axis=1)
...
def load_job_level_data():
    ...
    job_data["scheduled_time"] = pd.to_timedelta(job_data["scheduled_time"])

    base_date = pd.Timestamp("2025-01-01")

    job_data["scheduled_datetime"] = base_date + job_data["scheduled_time"]
    ...
    for column in [
        ...
        for window in rolling_windows:
            model_data[f"{column}_rolling_mean_{window}"] = (
                model_data[column].shift(1).rolling(window).mean()
            )

            model_data[f"{column}_rolling_std_{window}"] = (
                model_data[column].shift(1).rolling(window).std()
            )

            model_data[f"{column}_rolling_min_{window}"] = (
                model_data[column].shift(1).rolling(window).min()
            )

            model_data[f"{column}_rolling_max_{window}"] = (
                model_data[column].shift(1).rolling(window).max()
            )
...
def evaluate_job_level_regressors(
    job_model_data,
    job_feature_columns,
    split_index,
    job_regressors
):
    X_test = job_model_data[job_feature_columns].iloc[split_index:].copy()
    X_test = clean_feature_matrix(X_test)

    metrics_list = []

    for target_column, model in job_regressors.items():
        y_test = job_model_data[target_column].iloc[split_index:].values
        ...
        metrics = calculate_regression_metrics(
            y_true=y_test,
            y_pred=predictions,
            model_name="JobLevel_Internal_XGBoost",
...
def evaluate_job_type_classifier(
    job_model_data,
    job_feature_columns,
    split_index,
    classifier,
    ...
    y_test = job_model_data["job_type_encoded"].iloc[split_index:]

    if classifier is None:
        predictions = np.full(
            shape=len(y_test),
            fill_value=unique_classes[0]
        )
    else:
        predictions = classifier.predict(X_test)

    return {
        "model": "JobLevel_Internal_XGBoost",
        "target": "job_type",
...
def build_job_template_features(
    ...
    latest_job_features = (
        job_history_data[job_feature_columns]
        .iloc[-1]
        .copy()
    )
    ...
    time_of_day_minutes = window_row["hour"] * 60 + window_row["minute"]
...
def generate_jobs_for_test_horizon(
    ...
    for test_index, (_, window_row) in enumerate(test_windows.iterrows()):
        ...
        active_probability = float(active_probabilities[test_index])

        predicted_job_count = int(np.round(predicted_job_counts[test_index]))
        ...
        for job_number in range(predicted_job_count):
            feature_row = build_job_template_features(
                job_history_data=job_history_data,
            ...
            job_type_match = job_type_mapping[
                job_type_mapping["job_type_encoded"] == predicted_job_type_encoded
            ]
    ...
    return pd.DataFrame(generated_jobs)


# ============================================================
# EVALUATION AGGREGATION ONLY
# This is not the final optimization input.
# ============================================================

def aggregate_generated_jobs_for_evaluation(generated_jobs_data, test_windows):
...
def create_job_level_optimization_input_dataset(generated_jobs_data):
    if generated_jobs_data.empty:
        optimization_data = pd.DataFrame(
            columns=[
                ...
                "active_probability",
                "predicted_active_window",
                "predicted_job_count_for_window",
                "generated_job_number_in_window"
            ]
        )
    else:
        optimization_data = generated_jobs_data.copy()
        optimization_data = optimization_data.reset_index(drop=True)

        optimization_data["predicted_job_id"] = [
            f"pred_{index + 1:06d}"
            for index in range(len(optimization_data))
        ]

        optimization_data = optimization_data.rename(
            columns={
                DATE_COLUMN: "time_slot",
                "predicted_cpu_request": "required_cpu",
                "predicted_memory_request": "required_memory",
                "predicted_duration_minutes": "expected_duration_minutes",
                "predicted_job_type": "job_type",
                "predicted_job_type_encoded": "job_type_encoded",
                "predicted_active": "predicted_active_window"
            }
        )

        optimization_data["forecast_freq"] = FORECAST_FREQ
        optimization_data["forecasting_approach"] = "two_stage_job_level"
        ...
        for column in required_columns:
            if column not in optimization_data.columns:
                optimization_data[column] = 0

        optimization_data = optimization_data[required_columns]
        ...
        for column in numeric_columns:
            optimization_data[column] = pd.to_numeric(
                optimization_data[column],
                errors="coerce"
            ).fillna(0)

        optimization_data["required_cpu"] = optimization_data[
            "required_cpu"
        ].clip(lower=0)

        optimization_data["required_memory"] = optimization_data[
            "required_memory"
        ].clip(lower=0)

        optimization_data["expected_duration_minutes"] = optimization_data[
            "expected_duration_minutes"
        ].clip(lower=0)

    OPTIMIZATION_INPUT_DATA.parent.mkdir(parents=True, exist_ok=True)

    optimization_data.to_parquet(
        OPTIMIZATION_INPUT_DATA,
        index=False
    )

    print(f"Job-level optimization input dataset saved to: {OPTIMIZATION_INPUT_DATA}")
    print(f"Optimization input shape: {optimization_data.shape}")
    print(optimization_data.head())
...
def train_two_stage_job_level_model():
    ...
    window_feature_columns = build_window_feature_columns(window_model_data)
    ...
    job_internal_metrics = evaluate_job_level_regressors(
        job_model_data=job_model_data,
        job_feature_columns=job_feature_columns,
        split_index=job_split_index,
    ...
    job_type_internal_metrics = evaluate_job_type_classifier(
        job_model_data=job_model_data,
        job_feature_columns=job_feature_columns,
        split_index=job_split_index,
        classifier=job_type_classifier,
        unique_classes=unique_job_type_classes
    )
    ...
    job_history_data = job_model_data.iloc[:job_split_index].copy()
    ...
    optimization_data = create_job_level_optimization_input_dataset(
        generated_jobs_data=generated_jobs_data
    )

    print("Aggregating generated jobs only for evaluation...")

    aggregated_predictions = aggregate_generated_jobs_for_evaluation(
        generated_jobs_data=generated_jobs_data,
    ...
    job_internal_metrics_path = (
        METRICS_DIR
        / f"two_stage_job_level_internal_metrics_{FORECAST_FREQ}.csv"
    )

    job_type_internal_metrics_path = (
        METRICS_DIR
        / f"two_stage_job_level_job_type_internal_metrics_{FORECAST_FREQ}.csv"
    )
    ...
    pd.DataFrame(job_internal_metrics).to_csv(
        job_internal_metrics_path,
        index=False
    )

    pd.DataFrame([job_type_internal_metrics]).to_csv(
        job_type_internal_metrics_path,
        index=False
    )
    ...
    print(f"Window aggregation metrics saved to: {metrics_path}")
    print(f"Active classifier metrics saved to: {classifier_metrics_path}")
    print(f"Job-level internal metrics saved to: {job_internal_metrics_path}")
    print(f"Job type internal metrics saved to: {job_type_internal_metrics_path}")
    print(f"Job type mapping saved to: {job_type_mapping_path}")
    print(f"Final optimization input saved to: {OPTIMIZATION_INPUT_DATA}")

    print("\nWindow-level evaluation after generated jobs are aggregated:")
    print(pd.DataFrame(metrics_list))

    print("\nActive window classifier metrics:")
    print(pd.DataFrame([classifier_metrics]))

    print("\nJob-level internal regression metrics:")
    print(pd.DataFrame(job_internal_metrics))

    print("\nJob-type internal classification metrics:")
    print(pd.DataFrame([job_type_internal_metrics]))

    print("\nJob type mapping:")
    print(job_type_mapping)

    print("\nOptimization input preview:")
    print(optimization_data.head())
