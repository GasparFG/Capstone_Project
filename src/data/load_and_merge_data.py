...
df_batch_instance = pd.read_csv('data/raw/batch_instance.csv', names=[
...
df_batch = (
    df_batch_instance
    .merge(df_batch_task,   on=["job_id", "task_id"], how="left",
...
# - Normalise real_cpu_avg / real_mem_avg onto a common scale
#   (batch = absolute CPU units; interactive = utilisation ratio 0–1)
# - Slot conversion: start_timestamp, end_timestamp → start_slot, end_slot
# - Clip or drop negative durations (end < start)
# - Impute or drop rows with null capacity_cpu / capacity_memory
# - Filter killed / evicted batch instances by status (if needed)